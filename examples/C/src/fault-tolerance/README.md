...
- `Trigger` :
This triggers the segment to execute. If the `instantiate` port is triggered, execution starts from the first segment. If the `re_execute` port is triggered, it resumes from the indicated segment number passed by the port.
