...
legend_labels = [
    '1% Baseline', '1% Proposed', '1% Enhanced',
    '5% Baseline', '5% Proposed', '5% Enhanced',
    '10% Baseline', '10% Proposed', '10% Enhanced'
]
legend_colors = [
...
legend_hatches = ['/', '/', '/', 'O', 'O', 'O', 'X', 'X', 'X']

bar_colors = legend_colors
bar_hatches = legend_hatches
...
def plot_single_metric(ax, y_data, title, ylabel, value_format='int'):
    for i in range(len(x)):
        ax.bar(x[i], y_data[i], width=bar_width, color=bar_colors[i], alpha=0.85,
               hatch=bar_hatches[i], edgecolor='black')
        val = f'{int(y_data[i])}' if value_format == 'int' else f'{y_data[i]:.2f}'
        ax.text(x[i], y_data[i] + 1, val, ha='center', va='bottom', fontsize=18, rotation=(0 if value_format == 'int' else 45))
    ...
    for pos, label in zip(group_positions, failure_rate_labels):
        ax.text(pos, -0.05 * max(y_data), label, ha='center', va='top', fontsize=18, fontweight='bold')

    ax.set_xlim(-0.7, 9)
    ax.set_ylim(bottom=0, top=max(y_data) + (20 if value_format == 'float' else 200))
...
with PdfPages("examples/C/src/CheckpointRestore/evaluation/res420/graph.pdf") as pdf, \
    ...
    legend_elements = [Patch(facecolor=col, label=lab, hatch=hatch, edgecolor='black')
                       for col, lab, hatch in zip(legend_colors, legend_labels, legend_hatches)]
